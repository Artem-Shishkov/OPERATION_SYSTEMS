...
Изучить этапы компиляции программ на C++ с использованием GCC, исследовать влияние опций оптимизации на генерируемый ассемблерный код, освоить модульную структуру проекта, систему сборки Make, систему контроля версий Git, а также механизмы межпроцессного взаимодействия и синхронизации в ОС Linux.

## Задание

1. Написать программу на C++, вычисляющую числа Фибоначчи.
2. Выполнить трансляцию в ассемблерный код с различными уровнями оптимизации (`-O0`, `-O2`, `-Os`).
3. Проанализировать один из вариантов ассемблерного листинга, добавив комментарии (найти циклы, переменные, обращения к памяти).
4. Преобразовать программу в модульную (разделить на заголовочный файл и файлы реализации).
5. Разработать Makefile для сборки проекта и генерации ассемблерных листингов.
6. Усовершенствовать программу, добавив параллельный процесс средствами Linux (системные вызовы `fork`, `pipe`). Реализовать синхронизацию доступа к общему ресурсу (канал pipe).
7. Оформить проект с использованием Git.
